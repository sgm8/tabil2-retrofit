...
GPIO.setup(11, GPIO.OUT) #Approach relay
GPIO.setup(13, GPIO.OUT) #Arrival relay


print(is_allowed_time()) # print boolean to see if time is allowed. If having issues with this, check system time
previous_on = 0 # flag to check if light was on before

strobeOn()
time.sleep(5)
strobeOff()  # output & relay test. Turns light on for 5 sec then off. 

while True: 

    while is_allowed_time(): #runs if the time of day is between the hours set in config.json
        ...
        # turn on strobe if it hasn't been turned on already and if the countdown is greater than zero
        # turn off strobe if the countdown hits zero and it has been turned off before
        if countdown <= 0 and previous_on == 0:
            countdown = 0
        if countdown <= 0 and previous_on == 1:
            countdown = 0
            strobeOff()
            previous_on = 0
        if previous_on == 0 and countdown > 0 :
            strobeOn()
            previous_on = 1
        ...
        for train_id in arr_list:
            ...
            if bool1 and bool2:
                ...
                print("Train left arrival circuit")
        for train_id in brd_list:
            ...
            if bool1 and bool2:
                ...
                print("Train left platform circuit")
         
        print("Train Countdown: ", countdown)
        print("Max Time Countdown: ", countdown2)
